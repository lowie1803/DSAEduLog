...
void execute(int test_number)
{
  cin>>str;
  cntLtr.assign(26, 0);
  for (char c: str) cntLtr[c - 'a']++;
  string palinLeft = "", palinMid = "", palinRight = "", excessive = "";
  for (int i = 0; i < 26; i++) {
    for (int j = 0; j < cntLtr[i] / 2; j++) {
      palinLeft += ('a' + i);
    }
    if (cntLtr[i] & 1) {
      if (palinMid.size() == 0) {
  ...
  palinRight = palinLeft;
  reverse(palinRight.begin(), palinRight.end());
  string ans = excessive + palinLeft + palinMid + palinRight;
  cout << ans << "\n";
}
